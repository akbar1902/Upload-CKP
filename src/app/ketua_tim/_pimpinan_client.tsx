...
// ─── Main page ─────────────────────────────────────────────
export default function PimpinanKetuaTimDashboardClient() {
  ...
  const { data, isPending: queryPending, error: queryError, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        const newUsers = (usersRes.data as any[] || []).map((u) => {
          let timKerja = null;
  ...
  const filteredRows = useMemo(() => {
    ...
    if (statusFilter !== 'all') {
      result = result.filter(r => {
        ...
        if (statusFilter === 'submitted') {
            return r.upload?.status === 'submitted' || r.upload?.status === 'scored';
        }
    ...
    return result.sort((a, b) => {
      const getStatusScore = (upload: CKPUpload | null) => {
        ...
        if (upload.status === 'scored') return 1; // Sudah Dinilai
      ...
      <div className="p-4 lg:p-8 space-y-6 animate-fade-in">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h2 className="text-[22px] font-semibold tracking-tight"
              ...
              {user?.role === 'admin' ? 'Monitoring CKP (Admin)' : 'Dashboard Ketua Tim'}
            </h2>
        ...
        <div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4">
              {filteredRows.map(row => <PegawaiCard key={row.user.id} row={row} source="ketua_tim" />)}
            </div>
          )}
        </div>
